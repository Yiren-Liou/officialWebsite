        ...
        <div class="col col-md-7">
          <div class="card">
            ...
            <div v-else class="px-7 py-5">
              ...
              <div class="row mb-4">
                <div v-for="choice in donateChoice" :key="choice.id" class="col-md-4">
                  <input type="radio" class="btn-check" name="donateChoice" :id="`${choice.title}`" autocomplete="off">
                  <label class="btn btn-primary card-outline py-3 px-0 w-100" :for="`${choice.title}`">
                    <div class="d-columnCenter">
                      <h5 class="fw-bold fs-6 mb-1">{{ choice.title }}</h5>
                      <img :src="choice.img" class="mb-1" :alt="choice.price">
                      <span class="text-lighter d-block fs-9">已有 {{ choice.sponsorNum }} 人贊助</span>
                    </div>
                  </label>
                </div>
              </div>
              <div class="card-outline d-start px-5 py-4 mb-4">
                <label for="donatePrice" class="fw-bold fs-6 w-50">自訂捐款金額</label>
                <input type="number" v-model="donatePrice" class="form-control" id="donatePrice" placeholder="請輸入捐款金額" oninput="value=value.replace(/[^\d]/g,'');if(value<0)value=0">
              </div>
              <div class="d-center">
                <button
